...
impl WriteSpecial {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        let mut inner = match &self {
            ...
            WriteSpecial::ClearSerialErrorStatusRegister(clear_serial_status_register) => clear_serial_status_register.encode(),
        };
...
pub enum FileType {
    Text {size: u16, on_period: OnPeriod },
    String {size: u16},
    Dots { x: u8, y: u8, color_status: ColorStatus },
}
...
impl MemoryConfiguration {
    ...
    fn encode(&self) -> Vec<u8> {
        ...
        let mut file_size = match &self.file_type {
            FileType::Text { size, .. } | FileType::String { size, .. }  => format!("{size:0>4}").into_bytes(),
...
pub struct ConfigureMemory { //TODO check only the last file can have a size of 0
...
impl ConfigureMemory {
    ...
    pub fn new(configurations: Vec<MemoryConfiguration>) -> Result<Self, SignOutOfMemory> {
        for configuration in configurations.iter().rev().skip(1) {//TODO ignore for last element
            match configuration.file_type {
                FileType::Text { size, ..} | FileType::String { size }  => if size == 0 {return Err(SignOutOfMemory{})},
                _ => ()
            }
...
impl SetRunSequence {
    ...
    pub fn new(run_seqeunce_type: RunSequenceType, keyboard_accessible: bool, text_files: Vec<char>) -> Result<Self, TooManyTextFiles> {
...
pub enum RunDays {
    ...
    Always,
    Never,
    Range {start_day: time::Weekday, stop_day: time::Weekday}
}
...
impl RunDays {
    fn encode(&self) -> Vec<u8> {
        match &self {
            ...
            RunDays::Range { start_day, stop_day } => {
...
pub struct ClearSerialErrorStatusRegister { //TODO confirm whether this is correct, the
    //documentation sucks
